...
class Telegram {
    ...
    async listener() {
        ...
        this.bot.onText(/([0-9a-f]{24})/i, async (msg, match) => {
            ...
            if (!session || !match)
                return;
            const audio = await this.audio.get(new bson_1.ObjectID(match[1]));
            if (!audio || !audio._id) {
    ...
    async audioInfoCallback(msg, data) {
        if (!msg.message || !data[1])
            return;
        const audio = await this.audio.get(new bson_1.ObjectID(data[1]));
        if (!audio || !audio._id)
            return;
        this.bot.editMessageText(`ID: ${audio._id}\nTitle: ${audio.title}`, { chat_id: msg.message.chat.id, message_id: msg.message.message_id });
    }
    async playlistCallback(msg, data) {
        if (!msg.message)
            return;
        const view = await ((data[1]) ? this.genPlaylistView(parseInt(data[2], 10), new bson_1.ObjectID(data[1])) : this.genPlaylistView(parseInt(data[2], 10)));
    ...
    async listInfoCallback(msg, data) {
        if (!msg.message)
            return;
    ...
    async listCreateCallback(msg, data) {
        if (!msg.message || !data[1])
            return;
        const user = await this.getUser(msg.from.id);
        if (!user || !user._id.equals(new bson_1.ObjectID(data[1])))
            return;
        ...
        this.bot.onReplyToMessage(message.chat.id, message.message_id, reply => {
            if (!reply.from || reply.from.id !== msg.from.id)
                return;
            if (reply.text) {
    ...
    async listAudioAddCallback(msg, data) {
        if (!msg.message || !data[1])
            return;
        const list = await this.list.get(new bson_1.ObjectID(data[1]));
        const user = await this.getUser(msg.from.id);
        if (!user || !list || !list.owner.equals(user._id))
            return;
        if (data[2] === "done") {
            this.audioAddSession.delete(msg.message.chat.id);
            this.bot.editMessageText("Now this list have " + list.audio.length + " sounds!", { chat_id: msg.message.chat.id, message_id: msg.message.message_id });
        }
        else {
    ...
    async listAudioDeleteCallback(msg, data) {
        if (!msg.message || data.length < 3)
            return;
        if (data[3]) {
            this.list.delAudio(new bson_1.ObjectID(data[1]), new bson_1.ObjectID(data[2]));
            this.bot.editMessageReplyMarkup({ inline_keyboard: [[{ text: "Deleted", callback_data: "dummy" }]] }, { chat_id: msg.message.chat.id, message_id: msg.message.message_id });
        }
        else {
            const audioID = new bson_1.ObjectID(data[2]);
            const list = await this.list.get(new bson_1.ObjectID(data[1]));
    ...
    async listAudioCallback(msg, data) {
        if (!msg.message || data.length < 3)
            return;
        const view = await this.genAudioListView(new bson_1.ObjectID(data[2]), parseInt(data[3], 10) || 0, data[1] === "delete");
        if (!view)
            return;
        if (view.button) {
            this.bot.editMessageText(view.text, {
    ...
    async listRenameCallback(msg, data) {
        if (!msg.message || !data[1])
            return;
        const list = await this.list.get(new bson_1.ObjectID(data[1]));
        const user = await this.getUser(msg.from.id);
    ...
    async listDeleteCallback(msg, data) {
        if (!msg.message || !data[1])
            return;
        const list = await this.list.get(new bson_1.ObjectID(data[1]));
        const user = await this.getUser(msg.from.id);
        if (!user || !list || !list.owner.equals(user._id))
            return;
        if (data[2]) {
            this.list.delete(new bson_1.ObjectID(data[1]));
            this.bot.editMessageReplyMarkup({ inline_keyboard: [[{ text: "Deleted", callback_data: "dummy" }]] }, { chat_id: msg.message.chat.id, message_id: msg.message.message_id });
        }
        else {
    ...
    async processFile(msg, title) {
        ...
        try {
            ...
            if (audio)
                ...
                this.sendError(replyMessage, "failed");
        }
        catch (error) {
    ...
    async processLink(msg, link, title) {
        if (msg.from == null)
            return;
        const sender = await this.getUser(msg.from.id);
        if (!sender) {
